...
// newMovingAverageIteratorWithOptions returns an iterator for operating on a moving_average() call with minPeriods and center options.
func newMovingAverageIteratorWithOptions(input Iterator, n, minPeriods int, center bool, opt IteratorOptions) (Iterator, error) {
	// Calculate maxAge based on group by interval: window_size * interval
	var maxAge int64 = 0
	if !opt.Interval.IsZero() {
		maxAge = int64(opt.Interval.Duration) * int64(n)
	}
	...
	case FloatIterator:
		createFn := func() (FloatPointAggregator, FloatPointEmitter) {
			fn := NewFloatMovingAverageReducer(n)
			fn.minPeriods = minPeriods
			fn.maxAge = maxAge
			return fn, fn
		}
		iter := newFloatStreamFloatIterator(input, createFn, opt)
		if center {
			result = newCenterIterator(iter, n, opt)
		} else {
			result = iter
		}
	case IntegerIterator:
		...
		iter := newIntegerStreamFloatIterator(input, createFn, opt)
		if center {
			result = newCenterIterator(iter, n, opt)
		} else {
			result = iter
	...
	case UnsignedIterator:
		...
		iter := newUnsignedStreamFloatIterator(input, createFn, opt)
		if center {
			result = newCenterIterator(iter, n, opt)
		} else {
			result = iter
	...
	// Wrap with time range filter if needed
	return wrapWithTimeRangeFilter(result, opt), nil
}

// newMovingMedianIterator returns an iterator for operating on a moving_median() call.
func newMovingMedianIterator(input Iterator, n int, opt IteratorOptions) (Iterator, error) {
	switch input := input.(type) {
	case FloatIterator:
		createFn := func() (FloatPointAggregator, FloatPointEmitter) {
			fn := NewFloatMovingMedianReducer(n)
			return fn, fn
		}
		return newFloatStreamFloatIterator(input, createFn, opt), nil
	case IntegerIterator:
		createFn := func() (IntegerPointAggregator, FloatPointEmitter) {
			fn := NewIntegerMovingMedianReducer(n)
			return fn, fn
		}
		return newIntegerStreamFloatIterator(input, createFn, opt), nil
	case UnsignedIterator:
		createFn := func() (UnsignedPointAggregator, FloatPointEmitter) {
			fn := NewUnsignedMovingMedianReducer(n)
			return fn, fn
		}
		return newUnsignedStreamFloatIterator(input, createFn, opt), nil
	default:
...
// newMovingMedianIteratorWithOptions returns an iterator for operating on a moving_median() call with minPeriods and center options.
func newMovingMedianIteratorWithOptions(input Iterator, n, minPeriods int, center bool, opt IteratorOptions) (Iterator, error) {
	// Calculate maxAge based on group by interval: window_size * interval
	var maxAge int64 = 0
	if !opt.Interval.IsZero() {
		maxAge = int64(opt.Interval.Duration) * int64(n)
...
// newCenterIterator wraps an iterator to center the timestamps of emitted points.
func newCenterIterator(input Iterator, windowSize int, opt IteratorOptions) Iterator {
	if opt.Interval.IsZero() {
		return input // Can't center without interval
	}
	
	centerOffset := int64(opt.Interval.Duration) * int64(windowSize-1) / 2
	
	switch input := input.(type) {
	case FloatIterator:
		return &centerFloatIterator{
			input:        input,
			centerOffset: centerOffset,
		}
	case IntegerIterator:
		return &centerIntegerIterator{
			input:        input,
			centerOffset: centerOffset,
		}
	case UnsignedIterator:
		return &centerUnsignedIterator{
			input:        input,
			centerOffset: centerOffset,
		}
	default:
...
// centerFloatIterator centers timestamps for float points
type centerFloatIterator struct {
	input        FloatIterator
	centerOffset int64
}

func (c *centerFloatIterator) Next() (*FloatPoint, error) {
	p, err := c.input.Next()
	if p != nil {
		p.Time -= c.centerOffset
	}
	return p, err
}

func (c *centerFloatIterator) Stats() IteratorStats {
	return c.input.Stats()
}

func (c *centerFloatIterator) Close() error {
	return c.input.Close()
}

// centerIntegerIterator centers timestamps for integer points
type centerIntegerIterator struct {
	input        IntegerIterator
	centerOffset int64
}

func (c *centerIntegerIterator) Next() (*IntegerPoint, error) {
	p, err := c.input.Next()
	if p != nil {
		p.Time -= c.centerOffset
	}
...
func (c *centerIntegerIterator) Stats() IteratorStats {
	return c.input.Stats()
}

func (c *centerIntegerIterator) Close() error {
	return c.input.Close()
}

// centerUnsignedIterator centers timestamps for unsigned points
type centerUnsignedIterator struct {
	input        UnsignedIterator
	centerOffset int64
}

func (c *centerUnsignedIterator) Next() (*UnsignedPoint, error) {
	p, err := c.input.Next()
	if p != nil {
		p.Time -= c.centerOffset
	}
...
func (c *centerUnsignedIterator) Stats() IteratorStats {
	return c.input.Stats()
}

func (c *centerUnsignedIterator) Close() error {
	return c.input.Close()
}
...
// Time range filter iterators to trim results back to original time range

// timeRangeFilterFloatIterator filters FloatIterator results to original time range
type timeRangeFilterFloatIterator struct {
	input   FloatIterator
	minTime int64
	maxTime int64
}

func (itr *timeRangeFilterFloatIterator) Next() (*FloatPoint, error) {
	for {
		...
		// Only return points within original time range
		if p.Time >= itr.minTime && p.Time <= itr.maxTime {
			return p, nil
		}
	}
}

func (itr *timeRangeFilterFloatIterator) Close() error {
	return itr.input.Close()
}

func (itr *timeRangeFilterFloatIterator) Stats() IteratorStats {
	return itr.input.Stats()
}

// timeRangeFilterIntegerIterator filters IntegerIterator results to original time range
type timeRangeFilterIntegerIterator struct {
	input   IntegerIterator
	minTime int64
	maxTime int64
}

func (itr *timeRangeFilterIntegerIterator) Next() (*IntegerPoint, error) {
	for {
		p, err := itr.input.Next()
		if err != nil {
			return nil, err
...
func (itr *timeRangeFilterIntegerIterator) Close() error {
	return itr.input.Close()
}

func (itr *timeRangeFilterIntegerIterator) Stats() IteratorStats {
	return itr.input.Stats()
}

// timeRangeFilterUnsignedIterator filters UnsignedIterator results to original time range
type timeRangeFilterUnsignedIterator struct {
	input   UnsignedIterator
	minTime int64
	maxTime int64
}

func (itr *timeRangeFilterUnsignedIterator) Next() (*UnsignedPoint, error) {
	for {
		p, err := itr.input.Next()
		if err != nil {
			return nil, err
...
func (itr *timeRangeFilterUnsignedIterator) Close() error {
	return itr.input.Close()
}

func (itr *timeRangeFilterUnsignedIterator) Stats() IteratorStats {
	return itr.input.Stats()
}

// Helper function to wrap iterator with time range filter if needed
func wrapWithTimeRangeFilter(input Iterator, opt IteratorOptions) Iterator {
	// Only apply filter if original time range differs from current range
	if opt.OriginalStartTime != 0 && opt.OriginalEndTime != 0 &&
		(opt.OriginalStartTime != opt.StartTime || opt.OriginalEndTime != opt.EndTime) {
		
		switch itr := input.(type) {
		case FloatIterator:
			return &timeRangeFilterFloatIterator{
				input:   itr,
				minTime: opt.OriginalStartTime,
				maxTime: opt.OriginalEndTime,
			}
		case IntegerIterator:
			return &timeRangeFilterIntegerIterator{
				input:   itr,
				minTime: opt.OriginalStartTime,
				maxTime: opt.OriginalEndTime,
			}
		case UnsignedIterator:
			return &timeRangeFilterUnsignedIterator{
				input:   itr,
				minTime: opt.OriginalStartTime,
				maxTime: opt.OriginalEndTime,
			}
