...
func TestCompile_Failures(t *testing.T) {
	for _, tt := range []struct {
		...
		{s: `SELECT moving_average() from myseries`, err: `invalid number of arguments for moving_average, expected 2-4, got 0`},
		{s: `SELECT moving_average(value) FROM myseries`, err: `invalid number of arguments for moving_average, expected 2-4, got 1`},
		...
		{s: `SELECT moving_average(field1, 5, 0) FROM myseries`, err: `moving_average minPeriods must be at least 1, got 0`},
		{s: `SELECT moving_average(field1, 5, -1) FROM myseries`, err: `moving_average minPeriods must be at least 1, got -1`},
		{s: `SELECT moving_average(field1, 5, 2.0) FROM myseries`, err: `third argument for moving_average must be an integer, got *influxql.NumberLiteral`},
